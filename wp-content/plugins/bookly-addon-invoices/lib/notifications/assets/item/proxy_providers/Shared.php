...
abstract class Shared extends Proxy\Shared
{
    ...
    public static function prepareReplaceCodes( array $replace_codes, Codes $codes, $format )
    {
        ...
        if ( $codes->invoice_number ) {
            ...
            if ( $payment ) {
                $created = BooklyLib\Slots\DatePoint::fromStr( $payment->getCreated() );
                $replace_codes['{invoice_date}']     = BooklyLib\Utils\DateTime::formatDate( $created->format( 'Y-m-d' ) );
                $replace_codes['{invoice_due_date}'] = BooklyLib\Utils\DateTime::formatDate( $created->modify( $replace_codes['{invoice_due_days}'] * DAY_IN_SECONDS )->format( 'Y-m-d' ) );
