...
class Invoice extends BooklyLib\Base\Component
{
    ...
    public static function render( array $payment_data )
    {
        ...
        $created = BooklyLib\Slots\DatePoint::fromStr( $payment['created'] );
        $helper  = new Helper();
        $codes   = array(
            ...
            '{invoice_date}'      => BooklyLib\Utils\DateTime::formatDate( $created->format( 'Y-m-d' ) ),
            '{invoice_due_date}'  => BooklyLib\Utils\DateTime::formatDate( $created->modify( get_option( 'bookly_invoices_due_days' ) * DAY_IN_SECONDS )->format( 'Y-m-d' ) ),
